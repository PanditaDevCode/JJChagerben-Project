...
const CryptoItem = ({ ImageSrc, Title }) => {
  return (
    <section
      className="crypto container pt-[150px] pb-[70px] pl-0 pr-0 items-center bg-[#1b1b20] max-w-110vh
    ...
    >
      ...
      >
        <img
          src={ImageSrc}
          alt={`Imagen de ${Title}`}
          className="w-[500px] rounded-[20px] m-0 xxs:ml-[2px] xl:h-[20em] xl:w-[40em]"
        />
      </div>
      <div
      ...
      >
        ...
        <h2
          className="text-[40px] text-white font-bold leading-normal 
                                xxs:p-[10px] xxs:text-[30px] xxs:mr-[5px] xl:text-left "
        >
          ¿Qué son las{" "}
          <span className="text-black font-extrabold ">{Title}</span>?
        </h2>
        <p className=" font-bold text-white xl:text-left ">
          Una criptomoneda{" "}
          <span className=" text-black font-extrabold ">
            (también llamado criptoactivo o criptodivisa)
          </span>{" "}
...
const CryptoContent = () => {
  ...
  return <CryptoItem ImageSrc={BtcImg} Title={Title} />;
};
