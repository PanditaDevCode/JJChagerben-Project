export const HalvingInfo = ({ title, info, className = "" }) => {
  if (!info.year) {
    return (
      <div
        className={`calc-content rounded-lg border border-black p-4 ${className}`}
      >
        <h3 className="calc-text mb-2 text-lg font-semibold text-white">
          {title}
        </h3>
  ...
  return (
    <div
      className={`calc-content rounded-lg border border-black p-4 ${className}`}
    >
      <h3 className="calc-text text-white mb-2 text-lg font-semibold">
        {title}
      </h3>
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          <span className="calc-text text-white">Halving #</span>
          <span className="calc-text text-white font-medium">
            {info.number}
          </span>
        </div>
        <div className="flex justify-between">
